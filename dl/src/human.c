...
static const char power_letter[] = { 0, /* not used */
'K', /* kibi ('k' for kilo is a special case) */
'M', /* mega or mebi */
'G', /* giga or gibi */
'T', /* tera or tebi */
...
char *
human_readable(int n, char *buf, int opts, int from_block_size,
		int to_block_size)
{
	int inexact_style = opts
			& (human_round_to_nearest | human_floor | human_ceiling);
	unsigned int base = opts & human_base_1024 ? 1024 : 1000;
	int amt;
	int tenths;
	int exponent = -1;
	...
	char const *thousands_sep = "";
	//struct lconv const *l = localeconv ();
	size_t pointlen = 1; //strlen (l->decimal_point);
...
//    thousands_sep = l->thousands_sep;
	...
	/* Adjust AMT out of FROM_BLOCK_SIZE units and into TO_BLOCK_SIZE
	 ...
	 not use floating point operations.  */
//	myerror("n: %ld\n", n);
//	myerror("to_block_size: %ld\n", to_block_size);
//	myerror("from_block_size: %ld\n", from_block_size);
	if (to_block_size <= from_block_size) {
		if (from_block_size % to_block_size == 0) {
...
//  goto do_grouping;
	...
	use_integer_arithmetic: {
		...
		if (inexact_style == human_round_to_nearest ?
			...
			if ((opts & human_autoscale) && amt == base
	...
	if (opts & human_SI) {
		...
		if (exponent)
			*psuffix++ = (
					!(opts & human_base_1024) && exponent == 1 ?
							'k' : power_letter[exponent]);
